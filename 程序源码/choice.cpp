    ...
    QWidget(parent)
{
    ...
    //信号与槽是qt对象之间通信的接口
    //按下按钮执行sendslot发送信号
    connect(&b1, &QPushButton::released,this, &Choice::sendSlot1);

    b2.setParent(this);
    b2.setText("网络              链接网络用户到本地电脑     ");
    connect(&b2, &QPushButton::released,this, &Choice::sendSlot2);
    resize(400,300);

    b3.setParent(this);
    b3.setText("手机有线     通过数据线链接手机到本地电脑");
    connect(&b3, &QPushButton::released,this, &Choice::sendSlot3);


    b4.setParent(this);
    b4.setText("手机无线     无线链接手机到本地电脑     ");
    connect(&b4, &QPushButton::released,this, &Choice::sendSlot4);

    b5.setParent(this);
    b5.setText("Windows      Linux和Windows的同步");
    connect(&b5, &QPushButton::released,this, &Choice::sendSlot5);
    ...
    b5.setMaximumSize(400,50);
    b5.setMinimumSize(400,50);
...
//sendslot发送信号
void Choice::sendSlot1()
{
    emit mySignal1();
}

//sendslot发送信号
void Choice::sendSlot2()
{
    emit mySignal2();
}

void Choice::sendSlot3()
{
    emit mySignal3();
}

void Choice::sendSlot4()
{
    emit mySignal4();
}

void Choice::sendSlot5()
{
    emit mySignal5();
}
